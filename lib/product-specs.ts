/**
 * Static product spec metadata — keyed by product slug.
 *
 * Why static (not in Supabase)? Specs are physical attributes (dimensions,
 * material, etc.) that rarely change once a product is defined. Keeping them
 * in code means no extra Supabase column, no extra seed data, and no extra
 * admin UI for this prompt.
 *
 * Prompt 9 will add `specs JSONB` column to the `products` table when the
 * admin dashboard can edit them — then this file goes away.
 */
...
/** Get specs for a product by slug. Returns empty array if no specs defined. */
export function productSpecsBySlug(slug: string): ProductSpec[] {
  return SPECS[slug] ?? [];
}
